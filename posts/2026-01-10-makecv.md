...
<script type="module">
    import mikel from "/vendor/mikel.js";
    ...
    const getTemplate = name => {
        const templateUrl = `/templates/resume/${name}.mustache`;
        return window.fetch(templateUrl).then(response => {
    ...
    const app = kofi.emitter({
        [EVENTS.INIT]: () => {
            ...
            window.fetch("/templates/resume/templates.json")
                .then(response => response.json())
                .then(availableTemplates => {
                    app.state.templates = availableTemplates || [];
                    return getTemplate(app.state.data.template);
                })
                .then(templateContent => {
                    app.state.templateContent = templateContent;
                            ...
                            ${kofi.directives.when(app.state.activeLeftSideTab === LEFT_SIDE_TABS.LAYERS, () => kofi.html`
                                <div className="w-full py-4 overflow-y-scroll h-full">
                                    ${ui.LeftSideLayers({
                                        ...
                                        onDelete: element => {
                                            app.emit(EVENTS.DELETE_ELEMENT, { element });
                                        },
                                    })}
                                </div>
                            `)}
                        ...
                        <div className="w-full h-full py-4 px-2">
                            ...
                            ${kofi.directives.when(app.state?.data?.elements?.length > 0, () => kofi.html`
                                <div className="relative w-full h-full min-h-0">
                                    ...
                                    <div className="absolute z-100 bottom-0 right-0 mb-6 mr-4">
                                        <div className="bg-gray-100 rounded-full px-2 py-1 text-xs">Preview</div>
                                    </div>
                                    <div className="absolute z-100 bottom-0 left-0 mb-6 ml-4">
