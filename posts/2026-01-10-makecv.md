...
<script type="module">
    ...
    const ui = {
        ...
        LeftSideLayers: props => {
            ...
            const content = props.layers.map((layer, index) => {
                const className = kofi.classNames({
                    "absolute w-full rounded-lg overflow-hidden flex items-center select-none": true,
                ...
                return kofi.html`
                    <div key="${layer.id}" className="${className}" data-original-index="${index}" data-current-index="${index}" style="${layerStyle}">
                        ...
                        <div className="font-bold text-sm pr-3 py-2 w-full leading-none" onClick="${() => props.onClick(layer)}">
                            ${layer.name || layer.id}
                        </div>
                    </div>
                `;
    ...
    const app = kofi.emitter({
        ...
        [EVENTS.CREATE_ELEMENT]: elementType => {
            // TODO
        },
        [EVENTS.DELETE_ELEMENT]: elementId => {
            // TODO
        },
        ...
        [EVENTS.SET_ACTIVE_ELEMENT]: ({ element }) => {
            app.state.activeElement = app.state.data.elements.find(el => el === element || el.id === element) ?? null;
            app.emit(EVENTS.RENDER_EDITOR);
        },
