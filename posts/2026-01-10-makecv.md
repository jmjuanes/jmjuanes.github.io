    ...
    const ui = {
        ...
        Tabs: props => {
            const content = (props.items || props.tabs || []).map(tab => {
                const className = kofi.classNames({
                    "w-full text-center p-2 rounded-lg": true,
                ...
                return kofi.html`
                    <div className="${className}" onClick="${() => props.onChange(tab.id || tab.value)}">
                        <span className="font-bold">${tab.name || tab.text}</span>
                    </div>
                `;
            });
            return kofi.html`
        ...
        ElementsList: props => kofi.html`
            <div className="flex flex-col gap-4">
                ${Object.keys(ELEMENTS).map(key => kofi.html`
                    <div key="${key}" className="flex gap-3 items-center cursor-pointer" onClick="${() => props.onClick(key)}">
                            ...
                            <div className="w-full mb-4">
                                ${ui.Tabs({
                                    items: [
                                        ...
                                        { value: "elements", text: "Elements" },
                                    ],
                                    value: app.state.activeTab,
                                    onChange: tabValue => {
                                        app.state.activeTab = tabValue;
                                        app.emit(EVENTS.RENDER_EDITOR);
                                    },
                                })}
                            </div>
